...
async function exchangeStravaToken(code: string) {
  ...
  const tokenRes = await fetch('https://www.strava.com/oauth/token', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ client_id: process.env.STRAVA_CLIENT_ID, client_secret: process.env.STRAVA_CLIENT_SECRET, code, grant_type: 'authorization_code' }) });
  const tokenData = await tokenRes.json();
  if (!tokenRes.ok) { console.error('[strava/callback] token exchange failed:', tokenData); throw new Error('strava_token_failed'); }
  return tokenData;
}
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (mobileState) {
      ...
      const { error: updateError } = await supabase.from('profiles').update({ strava_access_token: access_token, strava_refresh_token: refresh_token, strava_expires_at: expires_at, strava_athlete_id: athlete?.id ?? null }).eq('id', mobileState.userId);
      if (updateError) { console.error('[strava/callback] mobile profile update failed:', updateError); return redirectToApp(mobileState.appRedirect, { error: 'strava_profile_update_failed' }); }
      return redirectToApp(mobileState.appRedirect, { success: 'strava_connected', sync: 'needed' });
    }

    const supabase = await createRouteSupabaseClient();
    const user = await requireUser(supabase);
    const { error: updateError } = await supabase.from('profiles').update({ strava_access_token: access_token, strava_refresh_token: refresh_token, strava_expires_at: expires_at, strava_athlete_id: athlete?.id ?? null }).eq('id', user.id);
    if (updateError) { console.error('[strava/callback] profile update failed:', updateError); return redirectWithParams({ baseUrl, returnTo, params: { error: 'strava_profile_update_failed' } }); }
    ...
    if (mobileState) return redirectToApp(mobileState.appRedirect, { error: message || 'unexpected_error' });
