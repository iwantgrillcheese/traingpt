...
function meta(totalWeeks: number): WeekMeta[] {
  ...
  return Array.from({ length: totalWeeks }, (_, i) => ({ label: `Week ${i + 1}`, phase: phases[i] ?? 'Base', startDate: iso(addWeeks(START, i)), deload: i > 0 && (i + 1) % 4 === 0 && ['Base','Build'].includes(phases[i]) }));
}
...
export async function GET() {
  ...
  return NextResponse.json({ pass: results.every(r => r.pass), passed: results.filter(r => r.pass).length, total: results.length, results });
}
