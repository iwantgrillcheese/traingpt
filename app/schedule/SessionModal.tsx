...
function LabelPill({ children, tone = "default" }: { children: React.ReactNode; tone?: "default" | "complete" | "hot" | "ready" }) {
  return (
    <span className={clsx("rounded-full border px-2.5 py-1 text-[12px] font-semibold", tone === "default" && "border-zinc-200 bg-white text-zinc-600", tone === "complete" && "border-[#BBD1FF] bg-[#EAF0FF] font-black text-[#2563FF]", tone === "hot" && "border-[#FFD7B8] bg-[#FFF3E9] font-black text-[#A34500]", tone === "ready" && "border-emerald-200 bg-emerald-50 font-black text-emerald-700")}>
      {children}
    </span>
  );
}
...
function WorkoutStructure({ segments }: { segments: Segment[] }) {
  return (
    <section className="rounded-[22px] border border-[#E7E9F1] bg-white shadow-[0_12px_30px_rgba(19,21,39,0.045)]">
      <div className="flex items-center justify-between gap-3 border-b border-[#E7E9F1] bg-[#FCFCFF] px-4 py-4"><h3 className="text-[16px] font-black tracking-[-0.035em] text-[#101114]">Workout structure</h3><span className="text-[12px] font-bold text-[#7A8093]">Original prescription</span></div>
      <div className="grid gap-2 p-4">{segments.map((segment, index) => <div key={`${segment.label}-${index}`} className="grid grid-cols-[70px_1fr_auto] items-center gap-3 rounded-[12px] border border-[#E7E9F1] bg-[#FCFCFF] p-3 text-[13px]"><strong className="tracking-[-0.02em] text-[#101114]">{segment.minutes}m</strong><span className="font-bold text-[#687085]">{segment.label}</span><span className="text-right font-black text-[#101114]">{segment.target}</span></div>)}</div>
    </section>
  );
}
