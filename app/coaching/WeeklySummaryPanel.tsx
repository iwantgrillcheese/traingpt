    ...
    <div className="rounded-2xl border bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="text-right">
          ...
          <p className={`text-lg font-semibold ${safePct == null ? 'text-zinc-400' : getColor(safePct)}`}>
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        <div className="rounded-xl bg-zinc-50 p-3">
          ...
          <p className="mt-1 text-sm font-semibold text-zinc-900">{totalPlanned}</p>
        </div>
        <div className="rounded-xl bg-zinc-50 p-3">
          <p className="text-xs text-zinc-500">Completed</p>
          <p className="mt-1 text-sm font-semibold text-zinc-900">{totalCompleted}</p>
        </div>
        <div className="rounded-xl bg-zinc-50 p-3">
      ...
      {/* By sport */}
      <div className="mt-4">
        <p className="text-sm font-medium text-zinc-900">By sport</p>

        {!hasBreakdown ? (
          <p className="mt-1 text-sm text-zinc-500">No sport breakdown available yet.</p>
        ) : (
            ...
            {breakdown.map(({ sport, completed, planned }) => {
              ...
              return (
                <li key={sport} className="flex items-center justify-between">
                  <div className="text-sm text-zinc-700">{sport}</div>
