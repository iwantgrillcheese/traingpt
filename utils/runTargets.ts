...
function sessionToText(session: RunTargetSession): string {
...
export function summarizeRunWeek(week?: WeekJson) {
  ...
  for (const [date, sessions] of Object.entries(week.days ?? {})) {
    ...
    for (const sess of (sessions ?? []) as RunTargetSession[]) {
      const sessionText = sessionToText(sess);
      if (!isRunSession(sessionText)) continue;

      hasRun = true;
      totalRunSessions += 1;

      const minutes = parseMinutesFromText(sessionText);
      if (minutes) {
        parseableRunSessions += 1;
        totalMin += minutes;
        longRunMin = Math.max(longRunMin, minutes);
      }

      if (isHardRun(sessionText)) dayHard = true;
    }
...
export function computeRunTargets(args: {
  ...
  if (race === "marathon" && !weekMeta.deload && weekMeta.phase !== "Taper") {
    ...
    const peakTargetByExp = exp === "beginner" ? 180 : exp === "advanced" ? 210 : 195;
