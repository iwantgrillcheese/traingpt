...
export function buildTriathlonWeekScaffold({
  ...
  const phase = phaseIntensity(weekMeta.phase, weekMeta.deload);
  const isFinalTaperWeek = phase === 'taper' && /week\s*(?:16|20|24)/i.test(weekMeta.label);
  ...
  // Every triathlon plan gets bricks. Race week keeps it very light or omits if final taper.
  if (!isFinalTaperWeek) {
    addSession(days, weekMeta.startDate, safeLongRideDay, {
      sport: 'run',
      title: 'Brick Run',
